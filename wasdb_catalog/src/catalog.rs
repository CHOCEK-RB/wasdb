...
impl Catalog {
    ...
    pub fn create_table(&mut self, table_name: String, schema: Schema, root_page_id: PageId) -> Result<(), CatalogError> {
    ...
    pub fn get_table(&self, table_name: &str) -> Result<&TableMetadata, CatalogError> {
        self.tables.get(table_name).ok_or_else(|| CatalogError::TableNotFound(table_name.to_string()))
    }
}
