...
impl<'a> Optimizer<'a> {
    ...
    pub fn optimize(
        &self,
        plan: LogicalPlan,
    ) -> Result<Box<dyn crate::executors::Executor>, String> {
        match plan {
            ...
            LogicalPlan::NestedLoopJoin {
                ...
                Ok(Box::new(crate::executors::NestedLoopJoinExecutor::new(
                    left_plan, right_plan, predicate, schema,
                )))
            }
            LogicalPlan::Filter {
                child,
                ...
                if let LogicalPlan::SeqScan { ref table_name } = *child
                    && let Some((col_name, val)) = index_match
                {
                    let table_meta = self
                        .catalog
                        .get_table(table_name)
